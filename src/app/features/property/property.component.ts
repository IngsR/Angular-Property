      ...
      <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <article class="lg:col-span-2 space-y-10">
            ...
            <header class="space-y-3 pb-6 border-b border-slate-200">
              <div class="flex flex-wrap items-center gap-2">
                <span class="px-3 py-1 rounded-full text-xs font-bold bg-slate-900 text-white">{{
                ...
                @if (property()!.partner?.verified) {
                  <span
                    class="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-200/60"
                  >
                    <svg
                ...
                @if (property()!.specification.certificate) {
                  <span
                    class="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold border border-slate-300 bg-white text-slate-700"
                  >
                    <svg
              ...
              @if (property()!.project) {
                <div
                  class="flex items-center gap-1.5 text-xs font-bold text-blue-700 bg-blue-50/80 px-3 py-1 rounded-lg w-fit"
                >
              ...
              <h1
                class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 tracking-tight leading-tight"
              >
                {{ property()!.title }}
              </h1>
              @if (property()!.tagline) {
              ...
              <div class="flex items-center gap-2 text-sm text-slate-600 pt-1">
                ...
                <span
                  ...
                  }}{{ property()!.location.city }}, {{ property()!.location.province }}</span
                >
              </div>
            </header>

            <!-- 2. Gallery -->
            <section aria-label="Galeri Foto Properti" class="space-y-4">
              ...
              <!-- Main Display -->
              <div
                class="relative h-[340px] sm:h-[460px] w-full rounded-2xl overflow-hidden bg-slate-950 group shadow-md"
              >
                <img
                ...
                <div class="absolute top-4 left-4 z-10">
                  <span
                    class="px-3 py-1 rounded-full bg-slate-900/80 backdrop-blur-md text-white text-xs font-bold uppercase tracking-wider"
                    >{{ currentGalleryImage().category }}</span
                  >
                </div>
                <button
                  type="button"
                  (click)="isViewerOpen.set(true)"
                ...
                @if (filteredGalleryImages().length > 1) {
                  <button
                    type="button"
                    (click)="prevImage()"
                    class="absolute left-3 top-1/2 -translate-y-1/2 p-2.5 rounded-full bg-black/50 hover:bg-black/80 text-white backdrop-blur-xs transition-colors"
                  >
                  ...
                  <button
                    type="button"
                    (click)="nextImage()"
                    class="absolute right-3 top-1/2 -translate-y-1/2 p-2.5 rounded-full bg-black/50 hover:bg-black/80 text-white backdrop-blur-xs transition-colors"
                  >
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path
                        stroke-linecap="round"
                ...
                <div
                  class="absolute bottom-4 left-4 right-4 flex items-center justify-between text-white text-xs z-10 drop-shadow-md"
                >
                  <p
                    class="font-medium bg-black/40 px-3 py-1 rounded-lg backdrop-blur-xs max-w-[80%] truncate"
                  >
                    {{ currentGalleryImage().alt }}
                  </p>
                  <span class="bg-black/60 px-2.5 py-1 rounded-lg font-bold"
                    >{{ activeImageIndex() + 1 }} / {{ filteredGalleryImages().length }}</span
                  >
                </div>
              </div>
                ...
                <div class="grid grid-cols-4 sm:grid-cols-6 gap-2 pt-1">
                  @for (img of filteredGalleryImages(); track img.id; let idx = $index) {
                    <button
                      ...
                      [class]="
                        'relative h-18 sm:h-22 rounded-xl overflow-hidden border-2 transition-all ' +
            ...
            <!-- 3. Specifications -->
            <section aria-label="Spesifikasi Bangunan" class="space-y-4">
              ...
              <dl class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                @for (spec of specificationList(); track spec.label) {
                  <div
                    class="p-3.5 bg-white rounded-xl border border-slate-200/80 shadow-xs flex flex-col justify-between space-y-2 hover:border-slate-300 transition-colors"
                  >
                    <div class="flex items-center gap-2 text-slate-400">
                      <span class="text-blue-600" [innerHTML]="spec.svg"></span>
                      <dt class="text-[11px] font-medium text-slate-500 truncate">
                        {{ spec.label }}
                      </dt>
                    </div>
            ...
            <!-- 4. Description -->
            <section aria-label="Deskripsi Properti" class="space-y-3">
              <h2 class="text-xl font-bold text-slate-900">Deskripsi &amp; Keunggulan Hunian</h2>
              <div
                class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs leading-relaxed text-slate-700 space-y-4 text-sm sm:text-base"
              >
                <p>{{ property()!.description }}</p>
                <div
                  class="pt-3 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-semibold text-slate-600"
                >
            ...
            @if (property()!.facilities && property()!.facilities.length > 0) {
              <section aria-label="Fasilitas Kawasan" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  @for (fac of property()!.facilities; track fac.id) {
                    <div
                      class="flex items-center gap-3 p-3.5 bg-white rounded-xl border border-slate-200/80 shadow-xs hover:border-slate-300 transition-colors"
                    >
                      <div
                        class="w-9 h-9 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center shrink-0"
                      >
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path
                            stroke-linecap="round"
            ...
            @if (property()!.floorPlans && property()!.floorPlans.length > 0) {
              <section aria-label="Denah Lantai Arsitektur" class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">
                      Eksplorasi blueprint 2D terukur, tata letak ruang, dan dimensi presisi setiap
                      ruangan
                    </p>
                  </div>
                  @if (property()!.floorPlans.length > 1) {
                    <div
                      class="flex items-center gap-1 bg-slate-100 p-1 rounded-full w-fit shrink-0"
                    >
                      @for (plan of property()!.floorPlans; track plan.id; let idx = $index) {
                        <button
                          type="button"
                          (click)="selectFloorPlan(idx)"
                          [class]="
                            'px-4 py-1.5 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 active:scale-95 ' +
                ...
                <div
                  class="bg-white rounded-3xl border border-slate-200/90 shadow-xs overflow-hidden"
                >
                  <div
                  ...
                  >
                    <div>
                      <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span
                          class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-blue-100 text-blue-800"
                          >Lantai {{ currentFloorPlan().floor }}</span
                        >
                      </div>
                    ...
                    <div
                      class="flex items-center gap-3 text-xs font-semibold text-slate-700 shrink-0"
                    >
                      <div
                        class="px-3 py-1 rounded-full bg-white border border-slate-200/80 shadow-2xs"
                      >
                        Luas: {{ currentFloorPlan().area }} m²
                      </div>
                      @if (currentFloorPlan().bedrooms) {
                        <div
                          class="px-3 py-1 rounded-full bg-white border border-slate-200/80 shadow-2xs"
                        >
                          {{ currentFloorPlan().bedrooms }} Kamar
                        </div>
                      }
                      @if (currentFloorPlan().bathrooms) {
                        <div
                          class="px-3 py-1 rounded-full bg-white border border-slate-200/80 shadow-2xs"
                        >
                          {{ currentFloorPlan().bathrooms }} KM
                        </div>
                      }
                    </div>
                  </div>
                  ...
                  <div class="p-4 sm:p-5 space-y-4">
                    <div
                      class="relative h-[320px] sm:h-[420px] rounded-xl overflow-hidden bg-slate-950 border border-slate-800 flex items-center justify-center select-none group"
                    >
                      <div
                        class="w-full h-full flex items-center justify-center p-4 transition-transform duration-200 ease-out"
                      ...
                      <div
                        class="absolute bottom-3 left-3 z-10 px-2.5 py-1 rounded-md bg-slate-900/80 text-white text-[11px] font-bold border border-slate-700/60"
                      >
                      ...
                      <div class="absolute top-3 right-3 flex gap-1 z-10">
                        <button
                          type="button"
                          (click)="zoomIn()"
                          class="p-1.5 rounded-full bg-slate-900/80 text-white hover:bg-blue-600 text-xs font-bold"
                        >
                          +
                        </button>
                        <button
                        ...
                        <button
                          type="button"
                          (click)="resetZoom()"
                          class="px-2 py-1 rounded-full bg-slate-900/80 text-white hover:bg-blue-600 text-[10px] font-bold"
                        >
                          1x
                        </button>
                      </div>
                    ...
                    @if (currentFloorPlan().rooms && currentFloorPlan().rooms!.length > 0) {
                      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 pt-2">
                        @for (room of currentFloorPlan().rooms!; track room.name) {
                          <div
                            class="p-2.5 rounded-xl border border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50/70 text-left"
                          >
                            <span class="text-xs font-bold text-slate-900 block truncate">{{
                              room.name
                            }}</span>
                            <div
                              class="flex items-center justify-between text-[11px] text-slate-600 font-semibold mt-1"
                            >
            ...
            <!-- 7. Location -->
            <section aria-label="Informasi Lokasi Kawasan" class="space-y-4">
              ...
              <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs space-y-5">
                <div
                  class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-100"
                >
                  <div>
                  ...
                  <div class="text-xs text-slate-600 bg-slate-100 px-3 py-1.5 rounded-lg w-fit">
                    Koordinat: {{ property()!.location.latitude.toFixed(4) }},
                    {{ property()!.location.longitude.toFixed(4) }}
                  </div>
                </div>

                <div
                  class="relative h-64 rounded-xl overflow-hidden bg-slate-800 border border-slate-700 flex flex-col items-center justify-center text-center p-6"
                >
                  <div class="relative z-10 space-y-2">
                    <div
                      class="w-12 h-12 rounded-full bg-blue-600/90 text-white flex items-center justify-center mx-auto shadow-lg ring-4 ring-blue-400/30"
                    >
                    ...
                    <h3 class="text-white font-bold text-sm">
                      Peta Kawasan {{ property()!.location.city }}
                    </h3>
                    <p class="text-xs text-slate-300 max-w-sm">
                      Lokasi strategis berada di titik koordinat ({{
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      @for (place of property()!.location.nearbyPlaces!; track place.name) {
                        <div
                          class="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200/60"
                        >
                          <div
                            class="w-8 h-8 rounded-lg bg-white shadow-xs text-blue-600 flex items-center justify-center shrink-0"
                          >
                            <svg
                              class="w-4 h-4"
                              fill="none"
          ...
          <div class="lg:col-span-1">
            <aside
              aria-label="Panel Pembelian dan Estimasi KPR"
              class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-lg space-y-6 sticky top-20"
            >
              <div class="space-y-1 pb-5 border-b border-slate-100">
                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider"
                  >Harga Penawaran Resmi</span
                >
                <div class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
                  {{ property()!.price | rupiah }}
                </div>
                <div
                  class="pt-2 flex items-center justify-between bg-blue-50/80 px-3.5 py-2 rounded-full border border-blue-100"
                >
                  <div>
                    <span class="text-[10px] font-medium text-slate-600 block"
                      >Estimasi Cicilan KPR</span
                    >
                    <span class="text-xs font-extrabold text-blue-700"
                      >~{{ estimatedMonthlyKpr() | rupiah: true }} / bulan</span
                    >
                  </div>
                  <button
                    type="button"
                    (click)="navigate('/simulator/kpr?price=' + property()!.price)"
                    class="text-[11px] font-bold text-blue-700 underline hover:text-blue-800"
                  >
                    Simulasikan
                  </button>
                </div>
              ...
              <div class="space-y-2.5">
                ...
                <div class="grid grid-cols-2 gap-2">
                  <button
                    ...
                    [class]="
                      'inline-flex items-center justify-center gap-2 font-semibold text-sm px-4 py-2 rounded-full border transition-all active:scale-[0.98] ' +
                      (isCompared()
                ...
                <div class="pt-4 border-t border-slate-100 space-y-3">
                  ...
                  <div class="space-y-2 pt-1 text-xs">
                    @if (property()!.partner!.phone) {
                      <div
                        class="flex items-center gap-2 text-slate-700 bg-slate-50 p-2.5 rounded-lg border border-slate-100"
                      >
                        <svg
              ...
              <div
                class="bg-slate-50 p-3.5 rounded-xl border border-slate-200/60 space-y-1.5 text-[11px] text-slate-600"
              >
                ...
                <p class="leading-relaxed">
                  Semua dokumen kepemilikan, sertifikat (SHM/PPJB), dan izin konstruksi telah
                  diverifikasi oleh tim legal platform.
                </p>
              </div>
            </aside>
          </div>
        ...
        @if (similarProperties().length > 0) {
          <section aria-label="Properti Serupa" class="pt-12 border-t border-slate-200 space-y-6">
            <div class="flex items-center justify-between">
              <div>
...
export class PropertyDetailPageComponent implements OnInit {
  ...
  readonly selectedCategory = signal<ImageCategory | 'ALL'>('ALL');
  readonly activeImageIndex = signal(0);
  readonly isViewerOpen = signal(false);
  readonly activeFloorIndex = signal(0);
  readonly zoomLevel = signal(1);

  readonly galleryCategories = computed<ImageCategory[]>(() => {
    const p = this.property();
    if (!p || !p.images) return [];
    return Array.from(new Set(p.images.map((img) => img.category)));
  });

  readonly filteredGalleryImages = computed<PropertyImage[]>(() => {
    const p = this.property();
    if (!p || !p.images) return [];
    const cat = this.selectedCategory();
    return cat === 'ALL' ? p.images : p.images.filter((img) => img.category === cat);
  });

  readonly currentGalleryImage = computed<PropertyImage>(() => {
    const images = this.filteredGalleryImages();
  ...
  readonly currentFloorPlan = computed<FloorPlan>(() => {
    const p = this.property();
  ...
  readonly estimatedMonthlyKpr = computed<number>(() => {
    const p = this.property();
    if (!p) return 0;
  ...
  readonly isCompared = computed<boolean>(() => {
    const p = this.property();
    return p ? this.compSvc.isInComparison(p.id) : false;
  });

  readonly specificationList = computed(() => {
    const s = this.property()?.specification;
    if (!s) return [];
    return [
      {
        label: 'Kamar Tidur',
        value: `${s.bedrooms} Kamar`,
      ...
      {
        label: 'Kamar Mandi',
        value: `${s.bathrooms} Kamar Mandi`,
        svg: '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>',
      },
      {
        label: 'Luas Bangunan',
        value: `${s.buildingArea} m²`,
        svg: '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>',
      },
      {
        label: 'Luas Tanah',
      ...
      {
        label: 'Parkir / Carport',
        value: `${s.parking} Mobil`,
        svg: '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>',
      },
      {
        label: 'Legalitas',
        value: s.certificate || 'SHM Murni',
      ...
      {
        label: 'Daya Listrik',
        value: s.electricity || '2200 VA',
        svg: '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>',
      },
      {
        label: 'Kondisi Perabotan',
        value:
          s.furnishing === 'FULL_FURNISHED'
            ? 'Full Furnished'
            : s.furnishing === 'SEMI_FURNISHED'
              ? 'Semi Furnished'
              : 'Unfurnished',
        svg: '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>',
      },
      {
        label: 'Arah Hadap',
        value:
          s.facing === 'NORTH'
            ...
            : s.facing === 'SOUTH'
              ...
              : s.facing === 'EAST'
                ? 'Timur'
                : 'Barat',
        svg: '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
      },
    ];
  });
  ...
  async loadProperty(slugStr: string): Promise<void> {
    ...
    try {
      const prop = await this.propertyService.getPropertyBySlug(slugStr);
      this.property.set(prop || null);
      if (prop) {
        const similar = await this.propertyService.getSimilarProperties(prop, 3);
  ...
  getCategoryCount(cat: ImageCategory): number {
    return (this.property()?.images || []).filter((i) => i.category === cat).length;
  }

  selectCategory(cat: ImageCategory | 'ALL'): void {
    this.selectedCategory.set(cat);
    this.activeImageIndex.set(0);
  }

  selectFloorPlan(idx: number): void {
    this.activeFloorIndex.set(idx);
    this.zoomLevel.set(1);
  }

  nextImage(): void {
    const total = this.filteredGalleryImages().length;
    if (total > 0) {
      this.activeImageIndex.update((curr) => (curr + 1) % total);
    }
  }
  ...
  prevImage(): void {
    const total = this.filteredGalleryImages().length;
    if (total > 0) {
      this.activeImageIndex.update((curr) => (curr - 1 + total) % total);
    }
  }
