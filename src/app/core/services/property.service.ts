...
export class PropertyService {
  ...
  async getProperties(query: PropertyQuery = {}): Promise<PropertySearchResult> {
    ...
    if (query.city && query.city !== 'ALL') {
      result = result.filter((p) => p.location.city.toLowerCase() === query.city!.toLowerCase());
    }
  ...
  async getFeaturedProperties(limit = 4): Promise<Property[]> {
    const items = this.hydratedProperties.filter((p) => p.featured).slice(0, limit);
    return this.simulateLatency(items);
  }
  ...
  async getCities(): Promise<string[]> {
    const cities = Array.from(new Set(this.hydratedProperties.map((p) => p.location.city))).sort();
