  ...
  template: `
    <main class="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-5">
      ...
      @if (properties().length === 0 && !loading()) {
        ...
        <!-- Page Header Bar -->
        <section class="bg-white p-4 sm:p-6 rounded-3xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-blue-700 shrink-0">
            ...
            <div>
              <h1 class="text-base sm:text-xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
                <span>Matriks Komparasi Properti</span>
                <span class="text-xs font-bold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200/60">
                  {{ properties().length }}/2
                </span>
              </h1>
        ...
        @if (properties().length < 2) {
          <div class="bg-blue-50/90 border border-blue-200 p-4 rounded-3xl text-xs text-blue-950 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
            <div class="flex items-center gap-2">
              <span class="text-base">💡</span>
              <span>Pilih <strong>1 properti lagi</strong> dari katalog untuk melihat perbandingan matriks 2 properti secara berdampingan.</span>
            </div>
            <button
              type="button"
              (click)="isAddModalOpen = true"
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-2 gap-3 sm:gap-6">
            <!-- Card 1 (Properti A) -->
            @if (propA()) {
              <div class="bg-white rounded-3xl border border-slate-200/90 p-3.5 sm:p-5 shadow-xs flex flex-col justify-between relative overflow-hidden">
                <button
                  type="button"
                ...
                <div class="space-y-3">
                  <div class="h-32 sm:h-52 rounded-2xl overflow-hidden bg-slate-100 relative">
                    <img [src]="propA()!.images[0]?.url" [alt]="propA()!.title" class="w-full h-full object-cover" referrerpolicy="no-referrer" />
                    <span class="absolute bottom-2 left-2 px-2.5 py-0.5 rounded-full bg-blue-600 text-white text-[10px] sm:text-xs font-black uppercase shadow-xs">Properti 1</span>
                  </div>
                  <div>
                    <span class="text-[10px] sm:text-xs font-bold text-blue-600 uppercase tracking-wider block truncate">{{ propA()!.propertyTypeName }}</span>
                    <h2 (click)="navigate('/property/' + propA()!.slug)" class="text-xs sm:text-base font-bold text-slate-900 line-clamp-2 hover:text-blue-600 cursor-pointer transition-colors">{{ propA()!.title }}</h2>
                    <p class="text-[11px] sm:text-xs text-slate-500 truncate mt-0.5">{{ propA()!.location.city }}</p>
                  </div>
                </div>
            ...
            @if (propB()) {
              <div class="bg-white rounded-3xl border border-slate-200/90 p-3.5 sm:p-5 shadow-xs flex flex-col justify-between relative overflow-hidden">
                ...
                <div class="space-y-3">
                  <div class="h-32 sm:h-52 rounded-2xl overflow-hidden bg-slate-100 relative">
                    <img [src]="propB()!.images[0]?.url" [alt]="propB()!.title" class="w-full h-full object-cover" referrerpolicy="no-referrer" />
                    <span class="absolute bottom-2 left-2 px-2.5 py-0.5 rounded-full bg-indigo-600 text-white text-[10px] sm:text-xs font-black uppercase shadow-xs">Properti 2</span>
                  </div>
                  <div>
                    <span class="text-[10px] sm:text-xs font-bold text-indigo-600 uppercase tracking-wider block truncate">{{ propB()!.propertyTypeName }}</span>
                    <h2 (click)="navigate('/property/' + propB()!.slug)" class="text-xs sm:text-base font-bold text-slate-900 line-clamp-2 hover:text-indigo-600 cursor-pointer transition-colors">{{ propB()!.title }}</h2>
                    <p class="text-[11px] sm:text-xs text-slate-500 truncate mt-0.5">{{ propB()!.location.city }}</p>
                  </div>
                </div>
              ...
              <!-- Slot 2 Empty -->
              <div class="bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 p-6 flex flex-col items-center justify-center text-center space-y-3">
            ...
            <div class="bg-white rounded-3xl border border-slate-200/90 overflow-hidden shadow-xs divide-y divide-slate-100">
              @for (attrA of getAttributes(propA()!); track attrA.key; let idx = $index) {
                <div class="p-3.5 sm:p-5 space-y-2.5">
                  <div class="flex items-center gap-2 text-xs sm:text-sm font-extrabold text-slate-800">
                  ...
                  <div class="grid grid-cols-2 gap-2.5 sm:gap-4 text-xs sm:text-sm">
                    <!-- Column 1 (Prop A) -->
                    <div [class]="'p-3 sm:p-4 rounded-2xl ' + (attrA.isHighlight ? 'bg-blue-50/80 text-blue-950 font-black border border-blue-100' : 'bg-slate-50 text-slate-800 font-semibold')">
                      <span class="text-[10px] text-blue-600 block font-bold mb-0.5 uppercase tracking-wider">Properti 1</span>
                      <div class="text-xs sm:text-base font-extrabold">{{ attrA.value }}</div>
                      @if (attrA.subtext) {
                        <div class="text-[10px] text-slate-500 font-normal mt-1 leading-tight">{{ attrA.subtext }}</div>
                      }
                    </div>
                    ...
                    @if (propB()) {
                      ...
                      <div [class]="'p-3 sm:p-4 rounded-2xl ' + (attrB.isHighlight ? 'bg-indigo-50/80 text-indigo-950 font-black border border-indigo-100' : 'bg-slate-50 text-slate-800 font-semibold')">
                        <span class="text-[10px] text-indigo-600 block font-bold mb-0.5 uppercase tracking-wider">Properti 2</span>
                        <div class="text-xs sm:text-base font-extrabold">{{ attrB.value }}</div>
                        @if (attrB.subtext) {
                          <div class="text-[10px] text-slate-500 font-normal mt-1 leading-tight">{{ attrB.subtext }}</div>
                        }
                      </div>
                    } @else {
                      <div class="p-3 sm:p-4 rounded-2xl bg-slate-50 text-slate-400 italic text-xs flex items-center justify-center border border-dashed border-slate-200">
      ...
      >
        <div class="space-y-3">
          @if (availableToAdd().length === 0) {
            <p class="text-sm text-slate-500 text-center py-6">Tidak ada properti lain yang tersedia dalam katalog.</p>
          } @else {
            @for (prop of availableToAdd(); track prop.id) {
              <div class="flex items-center justify-between p-3 rounded-2xl border border-slate-200 hover:border-blue-500 transition-colors">
                <div class="flex items-center gap-3">
                  <img [src]="prop.images[0]?.url" [alt]="prop.title" class="w-14 h-14 rounded-xl object-cover" />
                  <div>
                    <h4 class="text-sm font-bold text-slate-900">{{ prop.title }}</h4>
                    <p class="text-xs text-slate-500">{{ prop.location.city }} • {{ prop.price | rupiah }}</p>
                  </div>
                </div>
                <button
                  type="button"
