      ...
      <figure class="relative h-48 sm:h-52 w-full overflow-hidden bg-slate-100 m-0 shrink-0">
        ...
        <div class="absolute top-3 left-3 right-3 flex items-center justify-between gap-2 z-10">
          <div class="flex items-center gap-1.5 flex-wrap">
            <span class="text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-slate-900/85 text-white backdrop-blur-xs shadow-xs">{{ property.propertyTypeName }}</span>
            @if (property.partner?.verified) {
              <span class="text-[11px] font-semibold flex items-center gap-1 px-2.5 py-1 rounded-full bg-blue-600/90 text-white backdrop-blur-xs shadow-xs">
          ...
          <button
            ...
            [class]="'p-2 rounded-full backdrop-blur-md transition-all shadow-xs shrink-0 ' + (isFavorite ? 'bg-rose-500 text-white shadow-md scale-105' : 'bg-white/90 text-slate-700 hover:bg-white hover:text-rose-600')"
        ...
        <div class="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white text-xs font-medium z-10">
          <span class="flex items-center gap-1 drop-shadow-sm truncate max-w-[65%]">
            ...
            <span class="truncate">{{ property.location.district ? property.location.district + ', ' : '' }}{{ property.location.city }}</span>
          </span>
          <span [class]="'text-[11px] px-3 py-0.5 rounded-full font-bold backdrop-blur-xs shrink-0 ' + availabilityClass">{{ availabilityLabel }}</span>
        </div>
      </figure>
      ...
      <div class="p-4 sm:p-5 flex-1 flex flex-col justify-between bg-white space-y-3.5">
        <div class="space-y-3">
          <div class="flex items-center justify-between gap-2">
            @if (property.project) {
              <p class="text-[11px] font-bold text-blue-600 uppercase tracking-wider truncate flex items-center gap-1">
              ...
              <span class="text-[11px] font-semibold text-slate-600">Sertifikat {{ property.specification.certificate?.split(' ')[0] || 'SHM' }}</span>
            }
            @if (hasFloorPlan) {
              <span class="inline-flex items-center gap-1 text-[11px] font-bold text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded-full shrink-0 border border-indigo-100">
          ...
          <div>
            <h3 class="text-base font-bold text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-1 leading-snug" [title]="property.title">{{ property.title }}</h3>
            <p class="text-xs text-slate-500 line-clamp-1 mt-0.5">{{ property.location.address }}</p>
          </div>

          <!-- Specs Grid -->
          <dl class="grid grid-cols-4 gap-1 py-2 px-1 bg-slate-50 rounded-2xl border border-slate-100 text-slate-700">
            ...
            <div class="flex flex-col items-center justify-center p-1 text-center min-w-0 border-l border-slate-200/60">
              <dt class="sr-only">Kamar Mandi</dt>
              <dd class="flex items-center gap-1 text-xs font-bold text-slate-800">
                <svg class="w-3.5 h-3.5 text-slate-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
            ...
            <div class="flex flex-col items-center justify-center p-1 text-center min-w-0 border-l border-slate-200/60">
              ...
              <dd class="flex items-center gap-1 text-xs font-bold text-slate-800">
                <svg class="w-3.5 h-3.5 text-slate-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4"/></svg>
        ...
        <div class="bg-slate-50/90 rounded-2xl p-3 border border-slate-200/70 space-y-2">
          <div>
            <div class="flex items-center justify-between gap-1 mb-0.5">
              <span class="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Harga Jual (Tunai / KPR)</span>
              <span class="text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200/60">All-in</span>
            </div>
            <div class="text-lg sm:text-xl font-black text-slate-900 tracking-tight leading-tight">{{ property.price | rupiah }}</div>
          </div>
          <div class="pt-2 border-t border-slate-200/80">
            <div class="flex items-center justify-between gap-2">
              ...
              <span class="text-xs font-extrabold text-blue-700">~{{ estimatedMonthlyKpr | rupiah:true }}<span class="text-[10px] font-semibold text-slate-500">/bln</span></span>
            </div>
        ...
        <footer class="pt-2 border-t border-slate-100 flex items-center gap-2">
          <button
            ...
            [class]="'flex-1 py-2 px-2.5 rounded-full border text-xs font-bold transition-all flex items-center justify-center gap-1.5 shrink-0 active:scale-95 ' + (isCompared ? 'bg-blue-600 border-blue-600 text-white shadow-xs' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-slate-900')"
            [title]="isCompared ? 'Hapus dari perbandingan' : 'Tambah ke perbandingan'"
            aria-label="Bandingkan properti"
          >
            <svg class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3"/></svg>
            <span class="whitespace-nowrap text-[11px]">{{ isCompared ? 'Di Komparasi' : 'Bandingkan' }}</span>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <svg class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
          </button>
        </footer>
      </div>
    </article>
...
export class PropertyCardComponent {
  ...
  get primaryImageUrl(): string {
    return this.property.images?.[0]?.url || 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=800&q=80';
  }
