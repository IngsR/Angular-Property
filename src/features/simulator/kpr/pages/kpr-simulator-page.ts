...
@Component({
  ...
  template: `
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
      ...
      <!-- Hero Header -->
      <section aria-label="Header Simulator KPR" class="bg-slate-900 text-white rounded-3xl p-6 sm:p-10 relative overflow-hidden shadow-xl">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-5 space-y-6">
          <aside aria-label="Hasil Kalkulasi Angsuran" class="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
      ...
      @if (kprResult().schedule.length > 0) {
        <section aria-label="Tabel Amortisasi Tahunan" class="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-xs space-y-5">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
            <div>
              <h2 class="text-lg font-bold text-slate-900">Jadwal Amortisasi Pinjaman (Tahun ke Tahun)</h2>
              <p class="text-xs text-slate-500 mt-0.5">Rincian proporsi pokok angsuran vs beban bunga serta sisa pinjaman setiap akhir periode tahun.</p>
            </div>
            <button
              type="button"
            ...
            >
              ...
              <svg class="w-3.5 h-3.5" [class.rotate-180]="showSchedule()" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          @if (showSchedule()) {
            <div class="overflow-x-auto">
              <table class="w-full text-left text-xs text-slate-700 border-collapse">
                <caption class="sr-only">Rincian Amortisasi Tahunan KPR</caption>
                <thead>
                  <tr class="bg-slate-50 border-b border-slate-200 text-slate-900 font-bold uppercase tracking-wider text-[11px]">
                ...
                <tbody class="divide-y divide-slate-100">
                  @for (row of kprResult().schedule; track row.year) {
                    <tr class="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td class="py-3 px-4 text-emerald-600 font-semibold">{{ row.principalPaidYear | rupiah }}</td>
                      <td class="py-3 px-4 text-amber-600 font-semibold">{{ row.interestPaidYear | rupiah }}</td>
                      <td class="py-3 px-4 font-bold text-slate-800">{{ row.remainingPrincipal | rupiah }}</td>
                    </tr>
                  }
                </tbody>
              </table>
